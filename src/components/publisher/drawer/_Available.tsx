import { Trash2, Plus, CirclePlus } from "lucide-react";
...
const AvailableFields = ({ toggleDrawer }: { toggleDrawer: () => void }) => {
  ...
  const { availableFor, ctaText, ctaUrl, avatar } =
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files?.[0]) {
      try {
        ...
        dispatch(
          updateContent({ section: "available", data: { avatar: imageUrl } })
        );
          ...
          <div>
            ...
            {inputValues.length < 5 && (
              <button
                ...
                type="button"
              >
                <CirclePlus className="text-[#a020f0] ml-1 size-3" />

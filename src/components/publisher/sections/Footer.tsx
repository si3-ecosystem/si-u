...
const Footer = () => {
  const socialChannels = useSelector(
    (state: RootState) => state.content.socialChannels
  );
      ...
      <div className="max-w-[90rem] mx-auto space-y-2">
        ...
        <section className="flex flex-col md:flex-row justify-between items-center gap-3">
          ...
          <section className="flex items-center justify-between md:justify-end w-full gap-4">
            ...
            {/* Language Dropdown */}
            <div
              className="relative bg-[#C8BAFD] rounded-xl w-fit flex items-center gap-2"
              ref={dropdownRef}
            >
              <button
                type="button"
                ...
                >
                  {selectedRegion === null ? (
                    ...
                    <div className="text-start space-y-1 max-h-96 overflow-auto">
                      ...
                      {languagesByRegion[
                        selectedRegion as keyof typeof languagesByRegion
                      ].map((language: string) => (
                        <div
                          key={language}
                          onClick={() => {

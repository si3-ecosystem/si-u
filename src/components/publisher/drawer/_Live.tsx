...
const LiveFields = ({ toggleDrawer }: { toggleDrawer: () => void }) => {
  ...
  const reduxLive = useSelector(
    (state: RootState) => state.content.live
  ) as LiveData;
    ...
    if (e.target.files?.[0]) {
      try {
        ...
        if (!validateImage(file)) {
          toast.error(
            "Invalid image format or size. Please use JPG, JPEG, or PNG under 2MB."
          );
          return;
        }
        setIsUploading(true);
            ...
            <div className="flex flex-col items-center">
              ...
              >
                {isUploading ? (
                  ...
                  <div className="flex flex-col items-center gap-2">
                    <Upload className="text-purple-500 size-8" />
              ...
              {localLive.url && (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (localLive.image) {
                      dispatch(
                        updateContent({ section: "live", data: { url: "" } })
                      );
                    } else {
                      setValidation({
                        error: "You must have at least an image or a video URL",
          ...
          <div className="relative flex-1">
            <input
              ...
              onChange={(e) => {
                setLocalLive((prev) => ({
                  ...prev,
                  walletUrl: e.target.value,
                }));
                dispatch(
                  updateContent({
                    section: "live",
                    data: { walletUrl: e.target.value },
                  })
                );
              }}
        ...
        <section className="p-4 xl:p-6">
          <div className="flex justify-between items-center mb-3">
            <p className="block font-semibold">
              Supporting Media ({localLive.details.length}/6)
            </p>
          </div>
          {localLive.details.map((detail, index) => {
            ...
            return (
              <div
                key={index}
                className="relative border p-4 rounded-md mt-4 bg-white shadow-sm"
              >
                <Trash2
                  className={`absolute top-2 right-2 size-4 text-red-500 hover:text-red-700 ${
                    isMin ? "opacity-50 pointer-events-none" : ""
                  }`}
                  onClick={() => {
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleDetailChange(index, "url", e.target.value)
                  }
                  className={inputStyles}
                />
              </div>

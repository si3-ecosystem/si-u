...
const LandingFieldsComponent = ({
  ...
  const removeFromArray = useCallback(
    (field: keyof LandingTypes, index: number) => {
      const updatedArray = (localData[field] as string[]).filter(
        (_, i) => i !== index
      );
      setLocalData((prev) => ({ ...prev, [field]: updatedArray }));
      dispatch(
            ...
            <section key={field} className="px-4 py-2">
              ...
              {arr.map((item, index) => (
                ...
                >
                  ...
                  <Trash2
                    ...
                    onClick={() =>
                      removeFromArray(field as keyof LandingTypes, index)
                    }
                  />
                </div>
              ))}
              <div
                className={`flex gap-1 items-center mt-3 cursor-pointer text-xs w-max ${
                  isMax ? "opacity-50 pointer-events-none" : ""
                }`}
                onClick={() => {
                  if (!isMax) addToArray(field as keyof LandingTypes);
                }}
              >
                <CirclePlus className="text-[#a020f0] size-3 cursor-pointer" />
...
const LandingFields = dynamic(() => Promise.resolve(LandingFieldsComponent), {
  ssr: false,
});
